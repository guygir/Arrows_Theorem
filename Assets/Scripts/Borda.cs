    ...
    void Voting(){
        ...
        for(int i=0;i<voters.Length;i++){
            ...
            if(candidates.Length==3){
                if(chosenIndex!=0&&secondIndex!=0)
                    votes[0]+=3;
                if(chosenIndex!=1&&secondIndex!=1)
                    votes[1]+=3;
                if(chosenIndex!=2&&secondIndex!=2)
                    votes[2]+=3;
            }
            ...
            if(secondIndex==goalCandidate){
                votedForArrow[i]=2;
            }
            if(chosenIndex!=goalCandidate&&secondIndex!=goalCandidate){
                votedForArrow[i]=3;
            }
        }
    ...
    public void WinCheck(){
        ...
        for(int j=0;j<candidates.Length;j++){
            ...
            if(votes[j]<=votes[goalCandidate]){
                int min=0;
                if (candidates.Length==3){
                    min=Mathf.Min(votes[0],votes[1],votes[2]);
                }
                else{
                    min=Mathf.Min(votes[0],votes[1]);
                }
                achievedText.GetComponent<UnityEngine.UI.Text>().text =("Achieved: "+count.ToString()+", Min is: "+min);
                return;
            }
        }
